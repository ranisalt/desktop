(function (App) {
    ...
    var StreamInfo = Backbone.Model.extend({
        initialize: function () {
            ...
            engine.once('ready', function () {
                ...
                if (self.get('file_index')) {
                    size = engine.torrents[0].files[self.get('file_index')].length; // torrent with multi-files
                } else {
                    engine.torrents[0].files.forEach(function (file) { // pct torrent
                        size += file.length || 0;
                    });
                }
        ...
        updateStats: function () {
            ...
            this.set('pieces', engine.piecesGot);
            ...
            buffer_percent = swarm.progress * 100; //downloaded / (BUFFERING_SIZE / 100);
